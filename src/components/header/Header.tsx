...
class Header extends React.Component<unknown, HeaderState> {
  ...
  renderLateralMenu = (): JSX.Element => {
    ...
    return (
      <div
        id="lateral-menu"
        className={`${styles.lateralMenuWrapper} ${
          isMenuOpen ? styles.fadeIn : ''
        }`}
        role="button"
        tabIndex={0}
      ...
      >
        <div
          className={`${styles.lateralMenu} ${
            isMenuOpen && styles.lateralMenuOpen
          }`}
        >
          <Menu
    ...
    return (
      <div
        ...
        className={`${styles.dropdown} ${
          shouldShowDropdown ? styles.show : ''
        }`}
      >
        <div
          id={`${activeCategoryData.name}`}
            ...
            <div className={styles.category} key={subCategory.tracking_code}>
              ...
              <ul>
                {filterEmptyCategrories(subCategory.children).map(
                  (subSubCategory) => (
                    <li key={subSubCategory.tracking_code}>
                      {subSubCategory.name}
                    </li>
                  )
                )}
