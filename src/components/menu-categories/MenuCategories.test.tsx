...
describe('<MenuCategories />', () => {
  ...
  it('renders the correctly the category', () => {
    ...
    defaultProps.categories.forEach((category, idx) => {
      expect(wrapper.find(`.${styles.item}`).at(idx).text()).toEqual(category.name);
  ...
  it('should render the caret', () => {
    const wrapper = shallow(<MenuCategories {...defaultProps} hasCaret={true} />);
  ...
  it('should call function when click a category', () => {
    ...
    const wrapper = shallow(<MenuCategories {...defaultProps} handleCategory={clickFunction}/>);
