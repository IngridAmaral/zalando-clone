...
describe('<CampaignWrapper />', () => {
  ...
  it('renders bottom campaign componenet with correct props', () => {
    ...
    expect(wrapper.find(BottomCampaignWrapper).prop('brandName')).toEqual(
      brandName
    );
    expect(wrapper.find(BottomCampaignWrapper).prop('cardsBackground')).toEqual(
      cardsBackground
    );
    expect(wrapper.find(BottomCampaignWrapper).prop('fontColor')).toEqual(
      fontColor
    );
  });
});
